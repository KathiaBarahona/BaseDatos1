 ...
 app.post('/sintoma', function(req, res) {
     var connection = new sql.Connection(config, function(err) {
         if (err) {
             ...
             var request = connection.request();
             request.query("insert into Registro_Sintomas values ('"+req.body.id_registro+"','"+req.body.id_paciente+"','"+req.body.sintoma+"')", function(err, recordset) {
                 if (err) {
                     console.log("Error query");
                     res.status(400).end();
                 } else {
 ...
 app.post('/medicamento', function(req, res) {
     var connection = new sql.Connection(config, function(err) {
         if (err) {
             ...
             var request = connection.request();
             request.query("insert into Registro_Medicamentos values ('"+req.body.id_registro+"','"+req.body.id_paciente+"','"+req.body.medicamento+"')", function(err, recordset) {
                 if (err) {
                     console.log("Error query");
                     res.status(400).end();
                 } else {
